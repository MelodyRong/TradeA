document.write("<script src='http://www.jq22.com/jquery/jquery-1.10.2.js'></script>")
//关闭弹窗removeModa
function removeModa() {
    $('.myModa').fadeOut(300);
    setTimeout(function () {
        $('.myModa').remove()
    }, 300)
}
//定时弹窗myToast
function  myToast(message) {
    var top = $(window).height() * 0.3;
    $('body').append('<div class="myToast">' + message + '</div>');
    var top = ($(window).height() - $('.myToast').height()) / 2;
    var left = ($('body').width() - $('.myToast').width()) / 2;
    $('.myToast').css({'top': top + 'px', 'left': left + 'px','textAlign':'center'});
    setTimeout(function () {
        $('.myToast').fadeOut(300);
        setTimeout(function () {
            $('.myToast').remove()
        }, 300)
    }, 2000);
}
//提示弹框myConfirm
function myConfirm(options){
    var option = {
        title: "提示", message: "程序员太傻，忘记输入提示内容啦……", callback: function () {
        }
    };
    if (typeof(options) == "string") {
        option.message = options
    } else {
        option = $.extend(option, options)
    }
    var top = $(window).height() * 0.3;
    $('body').append('<div class="myModa"><div class="myAlertBox" style="margin-top:'
        + top
        + 'px"><h6>'
        + option.title
        + '</h6><p>'
        + option.message
        + '</p><div class="col2"><div class="col" style="margin-right: 20px;"><div class="ConfrimBtn exit">取消</div></div><div class="col"><div class="ConfrimBtn Confrimsure">确定</div></div></div></div></div>');
    $('body').css('overflow-y', 'hidden');
    $('.ConfrimBtn.exit').click(function () {
        removeModa();
        $('body').css('overflow-y', 'scroll')
    });
    $('.ConfrimBtn.Confrimsure').click(function () {
        removeModa();
        $('body').css('overflow-y', 'scroll');
        option.callback()
    })
}
//确认myAlert
function myAlert(options) {
    var option = {
        title: "提示", message: "程序员太傻，忘记输入提示内容啦……", callback: function () {
        }
    };
    if (typeof(options) == "string") {
        option.message = options
    } else {
        option = $.extend(option, options)
    }
    var top = $(window).height() * 0.3;
    $('body').append('<div class="myModa"><div class="myAlertBox"  style="margin-top:'
        + top
        + 'px"><h6>'
        + option.title
        + '</h6><p>'
        + option.message
        + '</p><div class="AlertBtn Alertsure">确定</div></div></div>');
    $('body').css('overflow-y', 'hidden');
    $('.AlertBtn.Alertsure').click(function () {
        removeModa();
        $('body').css('overflow-y', 'scroll');
        option.callback();
    })
}
//加载myLoadding
function myLoadding(message) {
    var m = message || '加载中，请稍后...';
    var top = $(window).height() * 0.3;
    $('body').append('<div class="myModa"><div class="myLoadding"><img src="data:image/gif;base64,"/><p>'
        + m
        + '</p></div></div>');
    var top = ($(window).height() - $('.myLoadding').height()) / 2;
    var left = ($('body').width() - $('.myLoadding').width()) / 2;
    $('.myLoadding').css({'top': top + 'px', 'left': left + 'px'})
}